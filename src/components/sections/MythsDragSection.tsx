...
const IGNORE_TAGS = new Set(["SCRIPT", "STYLE", "CODE", "PRE", "KBD", "SAMP", "TEXTAREA", "NOSCRIPT"])

const SHORT = "(?:в|к|с|у|о|и|а|но|да|на|по|за|из|от|до|об|обо|во|со|ко|не|ни|же|ли|бы)"

const RX_AFTER_SHORT = new RegExp(`(^|[\\s(«„"'])(${SHORT})\\s+(?=[\\p{L}\\d])`, "giu")

const RX_BEFORE_LAST_SHORT = new RegExp(`(\\S)\\s(${SHORT})([.!?:,…»"')\\]]*\\s*$)`, "giu")
    ...
    >
      ...
      <div className={styles.answer} ref={backRef} aria-hidden={!revealed}>
        <div className={styles.expertHeader}>
          {myth.expert.photo ? <img className={styles.avatar} src={myth.expert.photo} alt={fixText(myth.expert.name)} /> : <div className={styles.avatarPlaceholder} />}
          <div>
    ...
    () => {
      ...
      mm.add("(min-width: 812px)", () => {
        ...
        const tlIntro = gsap
          ...
          .to(intro, { opacity: 0, y: 0, ease: "none" })
  ...
  return (
    <section ref={sectionRef} className={styles.section}>
        ...
        <div ref={playgroundRef} className={styles.playground}>
          <div className={styles.left}>
            ...
            <div className={styles.cardSlot} style={{ position: "relative" }}>
              <div className={styles.card} ref={dragCardRef} style={desktopCardHidden ? { opacity: 0, pointerEvents: "none" } : undefined} aria-hidden={desktopCardHidden}>
                ...
                >
                  <div className={styles.dropHint} style={{ textAlign: "center" }}>
                    ...
                    <p style={{ textAlign: "center" }}>
                      {" "}
                      <span style={{ whiteSpace: "nowrap" }}>Ура! Вы развеяли все мифы&nbsp;—</span>
                      <br /> <span style={{ whiteSpace: "nowrap" }}>переходите к блоку ниже</span>{" "}
                    </p>
                  </div>
                </div>
              )}
      ...
      {!isMobile && (
        ...
        >
          <img ref={hintImgRef} className={styles.abs_434_img} src="/images/abs-4334.svg" alt="" draggable={false} />
          <p>
